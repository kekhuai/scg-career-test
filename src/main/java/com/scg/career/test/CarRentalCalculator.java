...
public class CarRentalCalculator {
    ...
    public void calculateLowestCost(List<Car> combinations, int seats) {
        if (seats < 1) {
            ...
            if (bestCombinations.size() == 0 || calculateCost(combinations) < calculateCost(
                    bestCombinations.stream().findFirst().orElseThrow(IllegalArgumentException::new))) {
                bestCombinations.clear();
                Collections.sort(combinations);
                bestCombinations.add(combinations);
            } else if (calculateCost(
                    bestCombinations.stream().findFirst().orElseThrow(IllegalArgumentException::new)) == calculateCost(
                            combinations)) {
                Collections.sort(combinations);
                bestCombinations.add(combinations);
            }
    ...
    public void displayBestCombinations() {
        bestCombinations.stream()
                .map(c -> c.stream().collect(Collectors.groupingBy(Function.identity(), Collectors.counting())))
                .forEach(i -> {
                    System.out.println(
                            i.entrySet().stream().map(j -> String.format("%s x %d", j.getKey().getSize(), j.getValue()))
                                    .reduce((first, second) -> first + ", " + second).orElse(""));
                });
        System.out.println("cost: "
                + calculateCost(bestCombinations.stream().findFirst().orElseThrow(IllegalArgumentException::new)));
    }
